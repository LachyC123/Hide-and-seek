...
var CFG={
  // catchRadius was 10 m, which is tighter than a phone can actually place two people.
  // Two handsets at +/-10 m read 20 m apart while their owners are touching.
  scatter:180, voteLen:60, catchRadius:20, conversion:60, locFresh:45,
  ...
  // GPS handling
  gpsForgive:1, catchSlop:1, catchSlopMax:30, maxAcc:120, maxSpeed:30, gpsQ:1.6,
  seekers:1,      // how many start the hunt
  zoneOut:15,     // seconds outside the circle before you are caught; -1 never
  staleAfter:30,  // after this long with no fix, a player is drawn as a last-known position
...
function applyCfg(c){
  ...
  ['scatter','catchRadius','conversion','zoneStages','tribunals','trackScale','trackRate',
   'hiderSight','nearWarn','seekers','zoneOut','objectives','gpsForgive','mapStyle'].forEach(function(k){ if(c[k]!==undefined) CFG[k]=c[k]; });
...
function sanitiseCfg(c){
  var d={matchLen:1800,areaR:450,imposters:1,scatter:180,catchRadius:20,conversion:60,seekers:1,zoneOut:15,
    fullSignal:60,zoneStages:5,zoneShrink:0.66,tribunals:2,trackScale:1,trackRate:1,
    hiderSight:1,nearWarn:0,seekers:1,zoneOut:15,objectives:1,gpsForgive:1,mapStyle:'real'};
  c=c||{};
  var o={
    ...
    catchRadius:clampN(+c.catchRadius||d.catchRadius,5,60),
    seekers:clampN(c.seekers===undefined?d.seekers:+c.seekers,1,4),
    zoneOut:(c.zoneOut===undefined?d.zoneOut:clampN(+c.zoneOut,-1,120)),
...
function assignRoles(ids,rng,imposters,volunteers,seekers){
  ...
  // there must always be someone left to hunt
  var want=clampN(seekers||1,1,Math.max(1,pool.length-1)),chosen=[];
  if(volunteers&&volunteers.length)
    for(i=0;i<pool.length&&chosen.length<want;i++)
      if(volunteers.indexOf(pool[i])>=0) chosen.push(pool[i]);
  for(i=0;i<pool.length&&chosen.length<want;i++)
    if(chosen.indexOf(pool[i])<0) chosen.push(pool[i]);
  for(i=0;i<chosen.length;i++) out[chosen[i]]='seeker';
  var imp=null;
  if(imposters>0&&pool.length>chosen.length+1){
    for(i=0;i<pool.length;i++){ if(chosen.indexOf(pool[i])<0){ imp=pool[i]; break; } }
    if(imp) out[imp]='imposter';
  }
  return {roles:out,imposterId:imp,seekerId:chosen[0],seekerIds:chosen};
}
...
/* Why the catch button is or is not showing. Silence was the worst part of this:
   a seeker standing next to someone got no button and no explanation, and could not
   tell a stale phone from being out of range. */
function catchStatus(seeker,players,nowMs,radius){
  if(!seeker||seeker.role!=='seeker'||seeker.caught) return {ok:false,why:'notseeker'};
  var best=null;
  Object.keys(players||{}).forEach(function(id){
    var q=players[id];
    if(!q||id===seeker.id||q.role==='seeker'||q.caught||q.lat==null) return;
    var d=metersBetween(seeker,q);
    if(!best||d<best.dist) best={target:q,dist:d};
  });
  if(!best) return {ok:false,why:'none'};
  var reach=catchDistance(seeker,best.target,radius);
  var out={target:best.target,dist:best.dist,reach:reach,ok:false,why:''};
  if(!seeker.locT||nowMs-seeker.locT>CFG.locFresh*1000){ out.why='you'; return out; }
  if(!best.target.locT||nowMs-best.target.locT>CFG.locFresh*1000){ out.why='them'; return out; }
  out.reveal=best.dist<=Math.max(60,reach*1.5);
  if(best.dist>reach){ out.why='far'; return out; }
  out.ok=true; return out;
}

/* --- dead reckoning ---------------------------------------------------
   A position crosses the network in one to two sync beats each way, so what one
   phone knows about another is ~3 seconds old — 10 m at running speed, which is
   most of the "the map is inaccurate" complaint. Remember the velocity each fix
   implies and carry the marker forward, capped so one bad fix cannot fling anyone. */
function velocityFrom(prev,next,dtSec){
  if(!prev||!next||prev.lat==null||next.lat==null||!(dtSec>0)) return {vE:0,vN:0};
  var d=metersBetween(prev,next);
  if(!(d>0.5)) return {vE:0,vN:0};
  var b=dirTo2(prev,next), sp=Math.min(d/dtSec,CFG.maxSpeed);
  return {vE:b.x*sp,vN:b.y*sp};
}
function predictPos(p,nowMs,maxAheadSec){
  if(!p||p.lat==null) return p;
  var vE=p.vE||0,vN=p.vN||0;
  if(!vE&&!vN) return {lat:p.lat,lng:p.lng};
  var age=Math.max(0,(nowMs-(p.locT||0))/1000);
  var t=Math.min(age,maxAheadSec===undefined?3:maxAheadSec);
  return offsetLatLng(p,vE*t,vN*t);
}

/* The host judges a catch with data one sync older than what the seeker's own
   screen verified a moment ago. Refusing what the seeker saw — silently — was the
   worst failure this game had: button pressed, nothing happens, nobody told why.
   So the host allows for its own staleness, and a genuine refusal answers back. */
function catchAccept(seeker,target,nowMs){
  if(canCatch(seeker,target,nowMs)) return true;
  if(!seeker||!target||seeker.id===target.id) return false;
  if(seeker.role!=='seeker'||seeker.converting||seeker.caught) return false;
  if(target.role==='seeker'||target.caught) return false;
  if(!seeker.locT||!target.locT) return false;
  var fresh=CFG.locFresh*2000;                    // twice the normal freshness window
  if(nowMs-seeker.locT>fresh||nowMs-target.locT>fresh) return false;
  return metersBetween(seeker,target)<=catchDistance(seeker,target)*1.6;
}
...
/* Being outside the circle now ends your hunt rather than merely lighting you up.
   The grace is short but not zero by default: GPS wobble at the boundary would
   otherwise knock people out while they are stood still just inside it. */
function zoneVerdict(p,zone,nowMs){
  var os=outsideState(p,zone,nowMs);
  if(!os.outside) return {outside:false,out:false,left:null,level:0};
  if(CFG.zoneOut<0) return {outside:true,out:false,left:null,level:os.level};
  var secs=p.outsideSince?Math.max(0,(nowMs-p.outsideSince)/1000):0;
  return {outside:true,out:secs>=CFG.zoneOut,left:Math.max(0,CFG.zoneOut-secs),level:os.level};
}
/* How long since this phone last said anything about where it is. */
function fixAge(p,nowMs){ return (!p||!p.locT)?Infinity:(nowMs-p.locT)/1000; }
function isStale(p,nowMs){ return fixAge(p,nowMs)>CFG.staleAfter; }
...
function makeGpsFilter(){
  return {
    pos:null, v:0, t:0, rejected:0, accepted:0, miss:0,
    push:function(fix){
      // fix: {lat,lng,acc,t}
      ...
      // Rejecting is how a player freezes on everyone else's map, so never reject for
      // long: after a few refusals in a row, take whatever the phone is offering.
      var stuck=(this.miss||0)>=3;
      if(!stuck&&acc>CFG.maxAcc&&d<400&&(fix.t-this.pos.t)<15000){
        this.rejected++; this.miss=(this.miss||0)+1; return this.pos; }
      if(!stuck&&d/dt>CFG.maxSpeed&&d>acc*2&&(fix.t-this.pos.t)<15000){
        this.rejected++; this.miss=(this.miss||0)+1; return this.pos; }
      this.miss=0;
...
var MP_DEFAULT={
  url:'https://acyepauwpqocbxvrqlcs.supabase.co',
  key:'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImFjeWVwYXV3cHFvY2J4dnJxbGNzIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODU2OTQzMDYsImV4cCI6MjEwMTI3MDMwNn0.KydC1wavzls4ovg_YyP7Nk1dv2x5rfZdShBigxhKFa4'
};
...
function mergePlayerInput(p,inp,nowMs){
  ...
  if(inp.lat!=null&&(inp.locT||0)>(p.locSrc||0)){
    ...
    var dtS=(p.locSrc&&inp.locT>p.locSrc)?(inp.locT-p.locSrc)/1000:0;
    var vel=(dtS>0.3&&dtS<10)?velocityFrom(p,inp,dtS):{vE:0,vN:0};
    p.vE=vel.vE; p.vN=vel.vN;
...
/* A phone that reloads gets a brand new id, so matching on id alone can never let
   anyone back in. If a player with the same name has gone quiet, that seat is theirs. */
function rejoinSlot(players,meId,name,nowMs){
  if(!players) return null;
  if(meId&&players[meId]) return meId;
  if(!name) return null;
  var want=String(name).toUpperCase(),hit=null;
  Object.keys(players).forEach(function(id){
    var p=players[id];
    if(hit||!p||p.bot) return;
    if(String(p.name||'').toUpperCase()!==want) return;
    if(presence(p,nowMs)==='live') return;         // that phone is still playing
    hit=id;
  });
  return hit;
}
...
/* --- pausing ------------------------------------------------------------
   Almost every deadline in a match is a wall-clock stamp, so a pause cannot just
   stop a counter — on resume, everything that was due has to move forward by the
   time nobody was playing. Miss one and it fires the instant play restarts. */
var DEADLINES=['fullSignalUntil','jammedUntil','missionNextAt','zoneCloseAt'];
var PLAYER_DEADLINES=['convertAt','exposedUntil','liveUntil','outsideSince','online','lastBlip','locT'];
function resumeShift(st,ms){
  if(!st||!(ms>0)) return st;
  var bump=function(o,k){ if(o&&typeof o[k]==='number'&&o[k]) o[k]+=ms; };
  bump(st,'t0');
  DEADLINES.forEach(function(k){ bump(st,k); });
  if(st.tribunal) bump(st.tribunal,'endsAt');
  if(st.closing) bump(st.closing,'at');
  (st.blips||[]).forEach(function(b){ bump(b,'until'); });
  Object.keys(st.players||{}).forEach(function(id){
    var p=st.players[id];
    PLAYER_DEADLINES.forEach(function(k){ bump(p,k); });
  });
  return st;
}
function isPaused(st){ return !!(st&&st.pausedAt); }

/* --- host succession -----------------------------------------------------
   Every phone reads the same room record, so they all compute the same heir and
   only one of them acts. Bots cannot host, and neither can a phone that has itself
   gone quiet. Nothing here needs a schema change: the new host writes the same row. */
function hostHeir(st,nowMs){
  if(!st||!st.players) return null;
  var ids=Object.keys(st.players).filter(function(id){
    var p=st.players[id];
    return p&&!p.bot&&id!==st.hostId&&presence(p,nowMs)!=='lost';
  });
  ids.sort();
  return ids.length?ids[0]:null;
}
function shouldClaimHost(st,meId,nowMs){
  if(!st||!meId||st.phase==='RESULTS') return false;
  if(hostAlive(st,nowMs)) return false;
  return hostHeir(st,nowMs)===meId;
}
...
function supaRequest(mp,path,opts){
  ...
  return {
    url:normaliseMpUrl(mp.url)+'/rest/v1/'+path,
...
/* An earlier version of the schema gave every operation its own function. Projects
   that ran it work exactly as well, so detect them and speak their dialect rather
   than sending someone back to the SQL editor. Both return the same shapes.
   Safe to delete once no project is still on the seven-function schema. */
function rpcLegacyFor(op,code,id,payload){
...
function rpcDialect(dialect,op,code,id,payload){
  return dialect==='legacy'?rpcLegacyFor(op,code,id,payload):rpcFor(op,code,id,payload);
}
/* PostgREST says PGRST202 both when the routine was never created and when it exists
   but the schema cache has not caught up yet — so this is not automatically "run the SQL". */
function isMissingFn(status,body){
  return status===404||/PGRST202/.test(body||'');
}
var MP_SQL=[
...
function supaErrorText(status,body){
  if(isMissingFn(status,body))
    return 'This project has not had the FALSE SAFE SQL run on it, or it ran but the API has not picked it up yet. Open Multiplayer setup, run the SQL block once, then try again in a few seconds.';
...
/* The dashboard shows the REST endpoint more prominently than the bare project URL,
   so that is what people copy. Take either, and quietly drop any API path off the end
   rather than rejecting a value that is only cosmetically wrong. */
function normaliseMpUrl(u){
...
function validMp(mp){
  return !!(mp&&typeof mp.url==='string'&&/^https:\/\/[^\s\/]+\.[^\s\/]+$/.test(normaliseMpUrl(mp.url))&&
...
function encodeMpConfig(mp){
  ...
  return encodeURIComponent(normaliseMpUrl(mp.url))+'~'+encodeURIComponent(mp.key);
}
function decodeMpConfig(s){
  if(!s||s.indexOf('~')<0) return null;
  var i=s.indexOf('~'),mp;
  try{ mp={url:normaliseMpUrl(decodeURIComponent(s.slice(0,i))),key:decodeURIComponent(s.slice(i+1))}; }
  catch(e){ return null; }
...
var CORE={mulberry32:mulberry32,metersBetween:metersBetween,offsetLatLng:offsetLatLng,CFG:CFG,
  ...
  canCatch:canCatch,catchAccept:catchAccept,catchStatus:catchStatus,zoneVerdict:zoneVerdict,
  velocityFrom:velocityFrom,predictPos:predictPos,
  fixAge:fixAge,isStale:isStale,rejoinSlot:rejoinSlot,eligibleVoters:eligibleVoters,resolveVote:resolveVote,voteConsequence:voteConsequence,
  ...
  resumeShift:resumeShift,isPaused:isPaused,hostHeir:hostHeir,shouldClaimHost:shouldClaimHost,
  supaRequest:supaRequest,rpcFor:rpcFor,rpcLegacyFor:rpcLegacyFor,rpcDialect:rpcDialect,
...
/* Somewhere to keep the profile between visits. The host bridge is used when a page
   provides it; otherwise localStorage, because on a static host there is nothing else
   and without it the player id is regenerated on every load — which is precisely why
   nobody could rejoin a match they had dropped out of. Falls back to memory so the
   game still runs where storage is blocked entirely. */
var STORE=(function(){
  if(window.storage&&window.storage.get) return {kind:'host',
    get:function(k){return window.storage.get(k,false).then(function(r){return r?r.value:null;})
      .catch(function(){return null;});},
    set:function(k,v){try{return Promise.resolve(window.storage.set(k,v,false));}catch(e){return Promise.resolve();}},
    del:function(k){try{return Promise.resolve(window.storage.delete&&window.storage.delete(k,false));}
      catch(e){return Promise.resolve();}}};
  var ls=null;
  try{ ls=window.localStorage; ls.setItem('fs:probe','1'); ls.removeItem('fs:probe'); }catch(e){ ls=null; }
  if(ls) return {kind:'local',
    get:function(k){try{return Promise.resolve(ls.getItem(k));}catch(e){return Promise.resolve(null);}},
    set:function(k,v){try{ls.setItem(k,v);}catch(e){} return Promise.resolve();},
    del:function(k){try{ls.removeItem(k);}catch(e){} return Promise.resolve();}};
  var mem={};
  return {kind:'none',
    get:function(k){return Promise.resolve(mem[k]===undefined?null:mem[k]);},
...
var NET={kind:'none',mp:null,lastErr:null,ok:0,fails:0,lastOkAt:0,dialect:null};
function mpFallback(saved){
  if(validMp(saved)) return saved;
  if(validMp(MP_DEFAULT)) return {url:normaliseMpUrl(MP_DEFAULT.url),key:MP_DEFAULT.key};
  return null;
}
...
function netRefresh(){
  var was=NET.mp&&NET.mp.key;
  NET.kind=netKind(); NET.mp=NET.kind==='supabase'?G.mp:null;
  if(!NET.mp||NET.mp.key!==was) NET.dialect=null;   // a different project may use a different schema
  return NET.kind;
}
...
/* Must never throw synchronously. Every caller does netSomething().then(...), and a
   synchronous throw skips the .catch entirely: createGame aborts half way through, and
   netLoop leaves netBusy stuck true so the room never syncs again. */
function supaCall(path,opts){
  var r,p;
  try{
    r=supaRequest(NET.mp,path,opts);
    if(typeof fetch!=='function') throw new Error('this browser cannot make network requests');
    p=fetch(r.url,{method:r.method,headers:r.headers,body:r.body,cache:'no-store'});
  }catch(e){ return Promise.reject(e); }
  return p.then(function(res){
      if(!res.ok) return res.text().then(function(t){
        var e=new Error(supaErrorText(res.status,t));
        e.status=res.status; e.missingFn=isMissingFn(res.status,t);
        throw e;
      });
...
function supaRpc(op,code,id,payload){
  var call=function(dialect){
    var c=rpcDialect(dialect,op,code,id,payload);
    if(!c) return Promise.reject(new Error('unknown room operation: '+op));
    return supaCall('rpc/'+c.fn,{method:'POST',body:c.args});
  };
  if(NET.dialect) return call(NET.dialect);
  return call('rpc').then(function(v){ NET.dialect='rpc'; return v; },function(e){
    if(!e||!e.missingFn) throw e;
    // no fs_rpc — the project may be on the older per-function schema
    return call('legacy').then(function(v){ NET.dialect='legacy'; return v; },function(e2){
      throw (e2&&e2.missingFn)?e:e2;      // neither exists: report the clearer message
    });
  });
}
...
function hostSim(){
  ...
  var n=now();
  // While paused nothing moves: no clock, no zone, no bots, no catches. _last is kept
  // current so the first tick after resuming is a normal-sized step, not a huge one.
  if(isPaused(st)){ st._last=n; return; }
  var t=(n-st.t0)/1000, ph=phaseFor(t,st.cfg), hc=huntClock(t);
  ...
  Object.keys(st.players).forEach(function(id){
    ...
    var zv=zoneVerdict(p,st.zone,n);
    if(zv.out){
      p.caught=true; p.convertAt=n+CFG.conversion*1000; p.outsideSince=null;
      pushEvent(st,'all','caught','OUT OF BOUNDS',p.name+' stayed outside the circle and is out.');
      pushEvent(st,id,'you-caught','OUT OF BOUNDS',"You left the safe zone. You're joining the hunt.");
      var wz=checkWin(st); if(wz) endMatch(st,wz);
      return;
    }
    p.stats.survived=hc;
  });
...
function applyAction(st,pid,a){
  ...
  if(a.type==='catch'){
    ...
    } else if(tgt&&!tgt.caught){
      pushEvent(st,pid,'miss','THAT ONE DIDN\'T COUNT',
        tgt.name+' had moved by the time your catch arrived. If you really tagged them, use \u201cI caught someone\u201d.');
    }
  }
  if(a.type==='claim'&&p.role==='seeker'&&!p.caught){
    // A seeker saying "I tagged them" outranks whatever the satellites think. Nothing
    // else works if the phones disagree about where two people standing together are.
    var ct=st.players[a.data];
    if(ct&&!ct.caught&&ct.role!=='seeker'&&ct.id!==p.id){
      ct.caught=true; ct.convertAt=now()+CFG.conversion*1000; ct.outsideSince=null;
      p.stats.catches++;
      pushEvent(st,'all','caught','HIDER CAUGHT',ct.name+' has been caught by '+p.name+'.');
      pushEvent(st,ct.id,'you-caught','CAUGHT!',"You're joining the hunt.");
      var wc=checkWin(st); if(wc) endMatch(st,wc);
    }
  }
  ...
  if(a.type==='pause'&&pid===st.hostId&&st.phase!=='LOBBY'&&st.phase!=='RESULTS'&&!st.pausedAt){
    st.pausedAt=now();
    pushEvent(st,'all','pause','MATCH PAUSED','Stop where you are. Nobody can be caught.');
  }
  if(a.type==='resume'&&pid===st.hostId&&st.pausedAt){
    resumeShift(st,now()-st.pausedAt); st.pausedAt=null; st._last=now();
    pushEvent(st,'all','resume','BACK ON','Go!');
  }
  if(a.type==='endnow'&&pid===st.hostId&&st.phase!=='RESULTS'){
    if(st.pausedAt){ resumeShift(st,now()-st.pausedAt); st.pausedAt=null; }
    endMatch(st,{winner:'stopped'});
  }
}
...
function startMatch(st){
  ...
  var r=assignRoles(ids,mulberry32(st.seed),st.cfg.imposters,volunteerIds(st.players),st.cfg.seekers);
...
function pushInput(){
  ...
  if(G.pos) G._pushedPos={lat:G.pos.lat,lng:G.pos.lng};
...
/* The old host's phone is gone. Carry on from the last state it published rather than
   ending everyone's match. Seeding lastSeq from st.ack is what stops the new host
   re-applying every action still sitting in the other phones' outboxes. */
function claimHost(){
  var st=G.st; if(!st||G.isHost) return;
  var gone=(st.players[st.hostId]||{}).name||'The host';
  G.isHost=true; G.outbox=[]; G.ack=0;
  st.hostId=G.me.id; st.hostAt=now(); st._last=now();
  G.lastSeq={};
  Object.keys(st.ack||{}).forEach(function(k){ G.lastSeq[k]=st.ack[k]; });
  var mine=st.players[G.me.id]; if(mine) mine.online=now();
  pushEvent(st,'all','host','MATCH RESCUED',gone+"'s phone dropped out. "+G.me.name+
    ' is running the match now — carry on.');
  G.flush=true;
  toast('You are hosting now');
}
function hostWatch(){
  if(G.mode!=='net'||G.isHost||!G.st||!G.code) return;
  if(shouldClaimHost(G.st,G.me.id,now())) claimHost();
}
...
function netStatus(){
  ...
  if(!G.isHost&&G.st&&G.st.hostAt&&!hostAlive(G.st,now()))
    return {s:'bad',t:shouldClaimHost(G.st,G.me.id,now())?'Taking over':'Host quiet'};
  if(since>6000) return {s:'warn',t:'Slow'};
  return {s:'ok',t:'Synced'};
}

function movedSincePush(){
  if(!G.pos) return false;
  if(!G._pushedPos) return true;
  return metersBetween(G._pushedPos,G.pos)>12;
}
function netLoop(){
  var n=now(), iv=G.isHost?CFG.syncHost:CFG.syncClient;
  if(G.mode==='net'&&G.code&&G.st&&!G.netBusy&&
     (G.flush||n-G.lastPush>=iv||(movedSincePush()&&n-G.lastPush>=600))){
    G.lastPush=n; G.flush=false; G.netBusy=true;
    var job;
    // a throw here rather than a rejection would leave netBusy true forever
    try{ job=G.isHost?pullInputs().then(pushState):pushInput().then(pullState); }
    catch(e){ job=Promise.reject(e); }
    var release=function(){ G.netBusy=false; };
    job.catch(netFail).then(release,release);
  }
...
/* ---------- keeping the phone awake ----------
   ...
   is hidden, so asking once at boot is close to useless — it has to be re-taken on
   every return, and the room resynced immediately rather than at the next poll. */
var WAKE={lock:null,want:false,granted:0,fails:0};
function keepAwake(want){
  if(want!==undefined) WAKE.want=!!want;
  if(!WAKE.want||WAKE.lock) return;
  if(!navigator.wakeLock||!navigator.wakeLock.request) return;
  if(document.hidden) return;
  try{
    navigator.wakeLock.request('screen').then(function(l){
      WAKE.lock=l; WAKE.granted++;
...
function pushLocalPos(){
  ...
  if(!G.pos) return;
  if(p.lat!=null) p.stats.dist+=metersBetween(p,G.pos);
  var dtS=(p.locT&&G.pos.t>p.locT)?(G.pos.t-p.locT)/1000:0;
  if(dtS>0.3&&dtS<10){ var v=velocityFrom(p,G.pos,dtS); p.vE=v.vE; p.vN=v.vN; }
  else if(dtS>=10){ p.vE=0; p.vN=0; }
  p.lat=G.pos.lat;p.lng=G.pos.lng;p.locT=G.pos.t;p.acc=G.pos.acc;
}
...
function initMap(){
  ...
  cv.addEventListener('touchend',function(){drag=null;pinch=null;});
  // pinch to zoom — the map used to frame itself and give you no say
  var pinch=null;
  cv.addEventListener('touchstart',function(e){
    if(e.touches.length===2){ pinch={d:touchGap(e),z:G.zoom}; drag=null; }
  },{passive:true});
  cv.addEventListener('touchmove',function(e){
    if(e.touches.length!==2||!pinch) return;
    var d=touchGap(e); if(!d||!pinch.d) return;
...
function setZoom(z){ G.zoom=clamp(z,0.35,5); touched(); }
function resizeMap(){
  if(!cv) return;
...
function drawBlips(st){
  ...
  (st.blips||[]).forEach(function(b){
    ...
    // how far away it is decides whether it is worth running at
    if(G.pos) label(s.x,s.y+r+12,Math.round(metersBetween(G.pos,b))+' m',
      b.leak?'#d3b6f5':'#f0a79a',8);
  });
}

/* Text alone is useless while running. An arrow that points at the circle, sized so
   it is readable at arm's length, is the difference between "get back inside" being
   an instruction and being a puzzle. */
function drawGuide(st,z){
  if(!G.pos) return;
  var c=toScreen(z), d=metersBetween(G.pos,z), edge=Math.max(0,d-z.r);
  var onScreen=(c.x>40&&c.x<VW-40&&c.y>60&&c.y<VH-90);
  var outside=d>z.r;
  if(!outside&&onScreen) return;                 // inside and can see it: nothing to say
  var mx=VW/2, my=VH/2, ang=Math.atan2(c.y-my,c.x-mx);
  var rad=Math.min(VW,VH)*0.34;
  var ax=mx+Math.cos(ang)*rad, ay=my+Math.sin(ang)*rad;
  var col=outside?'#f2a63b':'rgba(67,176,232,.85)';
  ctx.save();
  ...
  ctx.restore();
  label(ax,ay+26,outside?(Math.round(edge)+' m TO SAFETY'):(Math.round(d)+' m'),
    outside?'#f2a63b':'#cfc7e0',outside?10:8);
}
function drawNorth(){
  var x=VW-22,y=64;
  ctx.save();
  ctx.fillStyle='rgba(11,9,22,.6)'; ctx.fillRect(x-11,y-16,22,26);
  ctx.fillStyle='#e8503a'; ctx.beginPath();
  ctx.moveTo(x,y-12); ctx.lineTo(x-5,y-2); ctx.lineTo(x+5,y-2); ctx.closePath(); ctx.fill();
  ctx.fillStyle='#cfc7e0'; ctx.font='900 8px Helvetica,Arial,sans-serif'; ctx.textAlign='center';
...
function drawPlayers(st){
  ...
  ids.forEach(function(id){
    ...
    // your own marker uses your own live fix; everyone else is dead-reckoned a
    // moment ahead so runners stop being drawn where they were three seconds ago
    var tp=(id===G.me.id&&G.pos)?G.pos
      :(!isStale(p,now())&&(p.vE||p.vN))?predictPos(p,now(),3)
      :p;
    ...
    var stale=(id!==G.me.id)&&isStale(p,now());
    var alpha=p.caught?0.5:(stale?0.45:1);
    ...
    label(s.x,s.y-19*S,p.name.toUpperCase(),stale?'#9a92b5':col,9);
    if(p.caught) label(s.x,s.y+10,'CAUGHT','#e8503a',8);
    // Vanishing was worse than being wrong: people simply were not on the map. Show the
    // last place they were seen and say how old it is.
    else if(stale) label(s.x,s.y+10,'LAST SEEN '+mmss(fixAge(p,now())),'#9a92b5',8);
  });
...
function renderMap(){
  ...
  want=clamp(want/(G.zoom||1),0.12,6);
  G.view.mpp+=(want-G.view.mpp)*0.12;
  drawTerrain(z);
  drawZone(z,st);
  drawObjectives(st);
...
function saveProfile(){
  ...
  STORE.set('fs:profile',JSON.stringify({name:G.me.name,char:G.me.char,id:G.me.id,mp:G.mp}));
}
function loadProfile(){
  return STORE.get('fs:profile').then(function(v){
...
function saveMp(){
  ...
  var msg=$('#mpMsg');
  var mp={url:normaliseMpUrl(url),key:key};
  if(!validMp(mp)){
...
function renderResults(){
  ...
  releaseAwake();
  $('#resTitle').textContent=w==='imposter'?'Imposter victory':w==='seekers'?'Seekers win'
    :w==='stopped'?'Match stopped':'Hiders win';
  $('#resSub').textContent=w==='stopped'?'Called early':'Match over';
  $('#resLine').textContent=w==='imposter'?'They played everyone. The imposter turned the hunt against the hiders and was the last one standing.'
    :w==='seekers'?'Every single hider was hunted down.'
    :w==='stopped'?'The host ended the match early. Here is how far everyone got.'
    :'The hiders outlasted the hunt.';
...
function updateHUD(){
  ...
  if(st.phase==='SCATTER'){
    ...
    var zv=zoneVerdict(p,lerpZone(st),now());
    ttl='Outside safe zone';
    txt=CFG.zoneOut<0
      ? 'The seekers can see you out here. Get back inside the circle.'
      : 'GET BACK INSIDE \u2014 '+Math.ceil(zv.left==null?0:zv.left)+'s or you are OUT.';
    mood('zone');
  ...
  // catch button, plus an honest reason when there isn't one
  var cb=$('#catchbtn'),mb=$('#manualbtn'),cn=$('#catchnote');
  var amSeeker=(p.role==='seeker'&&!p.caught);
  if(amSeeker){
    var lp={id:p.id,role:'seeker',caught:false,acc:G.pos?G.pos.acc:null,
      lat:G.pos?G.pos.lat:p.lat,lng:G.pos?G.pos.lng:p.lng,locT:G.pos?G.pos.t:p.locT};
    var cs=catchStatus(lp,st.players,now(),CFG.catchRadius);
    if(cs.ok&&cs.target){
      var tid=cs.target.id;
      cb.classList.remove('hidden');
      cb.textContent='Catch '+cs.target.name.toUpperCase()+' · '+Math.round(cs.dist)+' m';
      cb.onclick=function(){ SFX.caught(); buzz(200); act('catch',tid); cb.classList.add('hidden'); };
      if(!G._catchBuzz){G._catchBuzz=1;buzz(60);}
      cn.textContent='';
    } else {
      cb.classList.add('hidden'); G._catchBuzz=0;
      cn.textContent=
        cs.why==='none'?'Nobody left to catch.'
        :cs.why==='you'?'Waiting for your own GPS — stand still for a moment.'
        :cs.why==='them'?(cs.target.name+"'s phone hasn't reported for a while. Use “I caught someone”.")
        :cs.why==='far'?(cs.reveal
            ?(cs.target.name+' is about '+Math.round(cs.dist)+' m away — get within '+
              Math.round(cs.reach)+' m.')
            :'No hider within reach. Follow the blips.')
        :'';
    }
    mb.classList.remove('hidden');
  } else { cb.classList.add('hidden'); mb.classList.add('hidden'); cn.textContent=''; G._catchBuzz=0; }

  // pause overlay — everyone sees it, only the host can lift it
  var pv=$('#paused');
  if(pv){
    var paused=isPaused(st);
    pv.classList.toggle('hidden',!paused);
    if(paused){
      var who=(st.players[st.hostId]||{}).name||'The host';
      $('#pausedBy').textContent=G.isHost
        ? 'You paused the game. Nobody can be caught and the zone is frozen.'
        : who+' paused the game. Nobody can be caught and the zone is frozen.';
      $('#pausedFor').textContent='paused for '+mmss((now()-st.pausedAt)/1000);
      $('#b-resume').classList.toggle('hidden',!G.isHost);
    }
  }

  // host controls
  var hb=$('#hostbtn');
  if(hb) hb.classList.toggle('hidden',!(G.isHost&&st.phase!=='RESULTS'&&!isPaused(st)));
...
/* The fallback that has to work when nothing else does: the seeker says who they
   tagged and that is the end of it. No distance check, no freshness check. */
function openManualCatch(){
  var st=G.st,here=G.pos;
  var ids=Object.keys(st.players).filter(function(id){
    var q=st.players[id];
    return q&&!q.caught&&q.role!=='seeker'&&id!==G.me.id;
  });
  if(!ids.length){ toast('Nobody left to catch'); return; }
  ids.sort(function(a,b){
    var pa=st.players[a],pb=st.players[b];
    var da=(here&&pa.lat!=null)?metersBetween(here,pa):1e9;
    var db=(here&&pb.lat!=null)?metersBetween(here,pb):1e9;
    return da-db;
  });
  modal('Who did you catch?','Tap their name. They are out straight away — no GPS needed.',
    ids.slice(0,8).map(function(id){
      var q=st.players[id];
      var d=(here&&q.lat!=null)?(' · about '+Math.round(metersBetween(here,q))+' m'):'';
      return {label:q.name.toUpperCase()+d,fn:function(){
        SFX.caught(); buzz(200); act('claim',id);
      }};
    }));
}
function openHostMenu(){
  modal('Host controls','You are running this match on your phone.',[
    {label:'Pause the match',fn:function(){ act('pause'); SFX.alarm(); }},
    {label:'End match now',fn:function(){
      modal('End the match?','Everyone goes straight to the results. This cannot be undone.',
        [{label:'Yes, end it',fn:function(){ act('endnow'); SFX.bad(); }}]);
    }}
  ]);
}
...
function joinGame(code){
  ...
  netGetState(code).then(function(s){
    ...
    var slot=rejoinSlot(s.players,G.me.id,G.me.name,now());
    if(slot&&slot!==G.me.id){ G.me.id=slot; saveProfile(); }   // take your old seat back
    var rejoin=!!slot;
...
function buildDev(){
  var groups=[
    ...
    ['IMPOSTER',[
      ...
      devButton('make me seeker',function(){ var p=me(); if(p){ p.role='seeker'; p.caught=false; } }),
...
function renderDevLog(){
  ...
  var lines=[
    ...
    'imposter '+(st.imposterId?(st.players[st.imposterId]||{}).name:'none')+(st.imposterEligible?'':' [EXPOSED]')+
      '  tips '+st.tips,
    'net '+netLabel().toLowerCase()+' '+ns.t.toLowerCase()+(NET.dialect?'/'+NET.dialect:'')+'  ok '+NET.ok+'  fail '+NET.fails+
...
function diagRows(){
  ...
  var awake=!!WAKE.lock;
  rows.push({s:awake?'ok':(navigator.wakeLock?'warn':'warn'),t:'Screen stays awake',
  ...
  rows.push({s:NET.kind==='none'?'warn':ns.s,t:'Multiplayer',
    d:NET.kind==='none'?'No transport set up. Create game will only offer solo play — open Multiplayer setup to connect a Supabase project.'
      :(netLabel()+' · '+ns.t+(NET.dialect?' · '+NET.dialect+' schema':'')+(G.code?' · room '+G.code:' · no room open')+
...
var SETUP=[
  ...
  {k:'seekers',t:'Seekers at the start',type:'seg',opts:[[1,'1'],[2,'2'],[3,'3']],
   h:'With six or more players one seeker is slow going until the first catch. Two gets a '+
     'big group moving straight away.'},
  ...
  {sec:'The zone'},
  {k:'zoneOut',t:'Leaving the circle',type:'seg',opts:[[0,'Out at once'],[15,'15s grace'],[-1,'Never out']],
   h:'Out at once ends your hunt the moment you step outside. 15 seconds allows for GPS '+
     'wobble at the edge. Never out only lights you up for the seekers — which is what let '+
     'people sit outside the circle all match.'},
...
function fetchStreets(centre,radius,force){
  ...
  useCache().then(function(cached){
    ...
    return overpassTry(centre,radius).then(function(res){
      ...
      STORE.set(key,JSON.stringify({ways:res.ways,centre:STREETS.centre,radius:radius}));
      return null;
    });
  }).catch(function(e){
...
function rulesSummary(c){
  ...
  var bits=[Math.round(c.matchLen/60)+' min',c.areaR+' m area',mmss(c.scatter)+' head start',
    ...
    ((c.seekers||1)>1?c.seekers+' seekers':'1 seeker'),
    (c.zoneOut<0?'zone is a warning only':c.zoneOut?'out '+c.zoneOut+'s after leaving':'out at once if you leave')];
  return bits.join(' · ');
}
...
function boot(){
  ...
  $('#b-diag-clear').onclick=function(){
    ...
    STORE.del(streetsCacheKey(c,Math.max(400,G.cfg.areaR+200)));
  ...
  $('#hZoomIn').onclick=function(){ SFX.tap(); setZoom((G.zoom||1)*1.5); };
  $('#hZoomOut').onclick=function(){ SFX.tap(); setZoom((G.zoom||1)/1.5); };
  $('#manualbtn').onclick=function(){ SFX.tap(); openManualCatch(); };
  $('#hostbtn').onclick=function(){ SFX.tap(); openHostMenu(); };
  $('#b-resume').onclick=function(){ SFX.ok(); act('resume'); };
